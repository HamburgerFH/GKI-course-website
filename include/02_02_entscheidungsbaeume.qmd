...
- Struktur zur Klassifikation oder Regression basierend auf Tests auf 
  Merkmale $X_j$ bzw. deren Werte $\Rightarrow$ typische Anwendungen:
...
- **interner Knoten**: Test auf Attributwert, z. B. $x_2 < 0{,}29$?
...
- horizontale und vertikale Linien: durch Entscheidungsbaum induzierte
  Schwellenwerte
- Hinweis zur Notation in der Abbildung: dort bezeichnen $x_1$ und $x_2$
  die beiden dargestellten Merkmale _**für einen beliebigen Datenpunkt i**_. In unserer allgemeinen Notation wäre
  ein Test für ein einzelnes neues Objekt $\bs{x}$ (Index $i$ weggelassen) entsprechend
  $x_2 < 0{,}53$.
...
- gegeben: Trainingsmenge $M = \{(\bs{x}_i, y_i)\}_{i=1}^N$ mit $p$
  Merkmalen
...
- sei $M = \{(\bs{x}_i, y_i)\}_{i=1}^N$ eine Trainingsmenge mit $K$ Klassen
- sei $A$ ein Attribut/Merkmal mit $v$ möglichen Ausprägungen
  $a_1, \dots, a_v$
- aufteilung von $M$ in Teilmengen $M_1, \dots, M_v$ gemäß Attribut $A$
- $M_j = \{ (\bs{x}_i, y_i) \in M : A(\bs{x}_i) = a_j \}$
...
- **_wie zuvor:_** $A$ ist Attribut/Merkmal mit $v$ Ausprägungen:
...
- es werden **binäre Partitionen**, d.h. Partitionen mit den Eigenschaften 
  $\mathcal{A}_i^L \cup \mathcal{A}_i^R = \mathcal{A}_i$, und 
