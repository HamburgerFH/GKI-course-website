...
- **Klassifikation:** $G$ ist qualitativ, z. B. Klassen `Ja/Nein`, `rot/blau`
...
- $\bs{x}_i \in \mathbb{R}^p$: Merkmalsvektor der $i$-ten Beobachtung
  (als Spaltenvektor), $i = 1, \dots, N$
- $\bs{X} \in \mathbb{R}^{N \times p}$: Datenmatrix mit den Beobachtungen
  als Zeilen, also mit $\bs{x}_i^T$ in der $i$-ten Zeile
- $X_j$: $j$-tes Merkmal bzw. $j$-te Spalte der Datenmatrix; $x_{ij}$ ist
  der konkrete Wert dieses Merkmals für Beobachtung $i$
- Hinweis: in der Entscheidungsbaum-Literatur wird ein ausgewähltes Merkmal
  häufig als Attribut $A$ bezeichnet. Dann meint $A$ keine neue
  Datenstruktur, sondern ein ausgewähltes Merkmal aus den Spalten von
  $\bs{X}$; $a_1, \dots, a_v$ bezeichnen mögliche Ausprägungen dieses
  Attributs.
...
- für den Iris-Datensatz gilt $p=4$, $N=150$
- $\bs{x}_i^T$ enthält alle vier Merkmale der $i$-ten Blüte in dieser
  Reihenfolge: `sepal_length` `sepal_width` `petal_length` `petal_width`
